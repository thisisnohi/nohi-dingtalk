...
DROP TABLE IF EXISTS DT_VOCATION_STATUS;
CREATE TABLE DT_VOCATION_STATUS
(
     ID VARCHAR(50) NOT NULL  COMMENT '主键'
    ,CREATED_BY VARCHAR(32) DEFAULT 'admin' NOT NULL  COMMENT '记录创建者'
    ,CREATED_TS DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL  COMMENT '记录创建时间'
    ,LAST_UPD_BY VARCHAR(32) DEFAULT 'admin' NOT NULL  COMMENT '记录最后更新者'
    ,LAST_UPD_TS DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL  COMMENT '记录最后更新时间'
    ,DURATION_UNIT VARCHAR(20) COMMENT '请假单位：percent_day：表示天；percent_hour：表示小时'
    ,DURATION_PERCENT INT COMMENT '假期时长*100，例如用户请假时长为1天，该值就等于100。'
    ,END_TIME DATETIME COMMENT '请假结束时间'
    ,START_TIME DATETIME COMMENT '请假开始时间'
    ,USERID VARCHAR(20) COMMENT '用户ID'
    ,LEAVE_CODE VARCHAR(50) COMMENT '假期类型'
    ,PRIMARY KEY (ID)
)  COMMENT '请假状态';
